...
function setupLabels(request) {
  ...
  if (!ghVersion.ok) {
    result('blocked', 'GitHub CLI (gh) is unavailable; install gh 2.0.0 or newer before GitHub label setup.');
    return;
  }
  const parsedGhVersion = versionFrom(ghVersion.stdout);
  if (!parsedGhVersion || !atLeast(parsedGhVersion, [2, 0, 0])) {
    result('blocked', 'GitHub label setup requires gh 2.0.0 or newer.');
    return;
  }
