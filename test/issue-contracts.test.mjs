...
async function installedValidator(t) {
  const root = await mkdtemp(join(tmpdir(), "repo-canon-issue-installed-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  for (const path of [
    "scripts/validate-issue-contract.mjs",
  ...
  return join(root, "scripts/validate-issue-contract.mjs");
}
...
test("runs from the exact installed workflow layout", async (t) => {
  const result = await exercise({
    validatorPath: await installedValidator(t),
    issue: {
      number: 42,
      body: "## Steps to reproduce\n\nRun the command.\n\n## Expected behavior\n\nIt exits.\n\n## Actual behavior\n\nIt hangs.",
      labels: [{ name: "bug" }, { name: "needs-triage" }],
